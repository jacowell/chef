...
class Chef
  module DSL
    module DeclareResource
      ...
      # Lookup a resource in the resource collection by name and remove it.  Returns
      ...
      def edit_resource!(type, name, run_context: self.run_context, &resource_attrs_block)
        find_resource!(type, name, run_context: run_context).instance_eval(&resource_attrs_block)
      end
      ...
      # for accumulator patterns.  In CRUD terminology this is ain "upsert" operation and is
      ...
      def edit_resource(type, name, created_at = nil, run_context: self.run_context, &resource_attrs_block)
        edit_resource!(type, name, run_context: run_context, &resource_attrs_block)
      rescue Chef::Exceptions::ResourceNotFound
      ...
      # found then the block will not be applied.
      #
      # @param type [Symbol] The type of resource (e.g. `:file` or `:package`)
      # @param name [String] The name of the resource (e.g. '/x/y.txt' or 'apache2')
      # @param run_context [Chef::RunContext] the run_context of the resource collection to operate on
      ...
      #   resource = find_resource(:service, 'whatever') do
      #     action :nothing
      #   end
      #
      def find_resource(type, name, run_context: self.run_context, &resource_attrs_block)
